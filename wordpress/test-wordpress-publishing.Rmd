...
I am using a special the R package RWordPress. This is a package for publishing blog posts from R to WordPress. The package bridges the gap between [R](https://www.r-project.org/) as an free software environment for statistical computing and graphics with the likewise open source website and blogging tool [WordPress](https://wordpress.org/). I will explain the procedure in R in another post with more details, but to get an overview have a look at this page by [Yihui Xie](https://yihui.name/knitr/demo/wordpress/).

For the demonstration I am using data from [Gapminder.org](http://www.gapminder.org/). Following is a quote from the Gapminder-about-page:
...
Different bubbles for different countries. The size of the bubbles represent the population.
